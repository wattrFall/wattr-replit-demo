...
import { mono, ThemeProvider, ContextualHelp, ThemeControl, Brand, Status, Metric, PageHead, Segmented, sentenceCase } from "./ui";
...
function Portfolio({ data }: { data: SessionData }) {
  ...
  return <Shell data={data}>
    <PageHead eyebrow={`Portfolio command / ${data.facilities.length} authorized`} title="Facility health" detail="Ranked operating context from your authorized sites." action={<Status>{formatSimulatedAt(simulatedAt).slice(0, 16)} UTC</Status>}/>
...
function Transparency({ facility, snapshot }: { facility: Facility; snapshot: CockpitSnapshot }) {
  return <section className="panel p-4">
    <div className="flex flex-wrap items-center justify-between gap-2">
      <div className="flex items-center gap-1"><div className="eyebrow">Data transparency</div><ContextualHelp title="Provenance, confidence, and limitations">
  ...
  return <Shell data={data} facility={facility}>
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,2.2fr)_minmax(300px,.72fr)]">
      <section className="panel min-w-0 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 p-5"><div><h2 className="font-semibold">Synchronized facility twin</h2><p className="mt-1 text-xs text-slate-500">Select an asset to inspect its replay state and dependencies.</p></div><div data-guide="view" className="flex items-start gap-2"><Segmented label="Twin view" value={view} onChange={(item) => { setView(item); if (item === "thermal") guidance.emit("thermal-view"); }} options={[{ value: "physical" as TwinView, label: "Physical" }, { value: "thermal" as TwinView, label: "Thermal overlay" }]}/><ContextualHelp title="About twin views"><p>Physical view emphasizes equipment. Thermal overlay colors modeled inlet temperatures to reveal hot spots; it does not imply measured telemetry.</p></ContextualHelp></div></div>
...
function AssistantPage({ data, facility }: { data: SessionData; facility?: Facility }) {
  ...
  return <Shell data={data} facility={facility}>
    <PageHead
      eyebrow={`Ask Wattr / ${ROLE_LABELS[data.me.role]}`}
