...
import { formatSimulatedAt, SCENARIO_START_S, useScenarioSession } from "@/lib/cockpit/session";
...
type Audit = { id: number; action: string; scenario_id: string; simulated_at: number; model_version: string; payload: { recommendationId?: string; outcome?: string }; created_at: string };
...
function Portfolio({ data }: { data: SessionData }) {
  return <Shell data={data}><PageHead eyebrow={`PORTFOLIO COMMAND / ${data.facilities.length.toString().padStart(2, "0")} AUTHORIZED`} title="Facility health" detail="Only facilities permitted by your server-side grants appear here."/><div className="grid gap-3 md:grid-cols-4"><Metric label="Sites monitored" value={String(data.facilities.length).padStart(2, "0")} sub="permission-filtered"/><Metric label="Fleet headroom" value="184" unit="kW" sub="synthetic scenario"/><Metric label="Open incidents" value="01" sub="requires review" warn/><Metric label="Fleet PUE" value="1.199" sub="simulated"/></div><section className="panel mt-5 overflow-hidden"><div className="border-b border-slate-800 px-5 py-4"><h2 className="font-semibold">Authorized sites</h2></div>{data.facilities.map(f => <button key={f.id} onClick={() => navigate(`/facilities/${f.id}/operations`)} className="facility-row"><div><b>{f.name}</b><div className="mt-1 text-xs text-slate-500">{f.location}</div></div><div className="flex items-center gap-4"><Status tone="warn">Watch</Status><span className={`${mono} text-[10px] text-slate-500`}>{f.provenance}</span><ArrowRight size={16}/></div></button>)}{data.facilities.length === 0 && <p className="p-6 text-sm text-slate-500">No facility grants are assigned to this account.</p>}</section></Shell>;
}

function useClock() {
  const state = useScenarioSession();
  useEffect(() => {
    if (!state.playing) return;
    const timer = window.setInterval(() => state.advance(state.speed), 1000);
    return () => clearInterval(timer);
  }, [state.playing, state.speed, state.advance]);
  const elapsed = state.simulatedAt - SCENARIO_START_S;
  const ramp = Math.min(1, Math.max(0, elapsed / 1800));
  return { ...state, itKw: Math.round(1640 + 202 * ramp), peakC: 28.6 + 3.2 * ramp, pue: 1.17 + .029 * ramp };
}

function Operations({ data, facility }: { data: SessionData; facility: Facility }) {
  const s = useClock();
  return <Shell data={data} facility={facility}><PageHead eyebrow={`FACILITY / ${facility.id.toUpperCase()} / OPERATIONS`} title={facility.name} detail={`${facility.location} · deterministic GPU Training Ramp · ${facility.provenance}`} action={<div className="flex gap-2"><button className="button secondary" onClick={() => s.setPlaying(!s.playing)}>{s.playing ? <Pause size={15}/> : <Play size={15}/>} {s.playing ? "Pause" : "Play"}</button><button className="button secondary" onClick={s.reset}><RotateCcw size={15}/>Reset</button></div>}/><div className="mb-4 flex flex-wrap items-center gap-3 border-y border-slate-800 py-2.5"><span className={mono}>{formatSimulatedAt(s.simulatedAt)}</span><div className="ml-auto flex gap-1">{([1,5,10,30,60] as const).map(v => <button key={v} onClick={() => s.setSpeed(v)} className={`speed ${s.speed === v ? "selected" : ""}`}>{v}×</button>)}</div><Status tone="warn">{s.mode} mode</Status></div><div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5"><Metric label="IT power" value={s.itKw.toLocaleString()} unit="kW" sub="workload-driven"/><Metric label="Total facility" value={Math.round(s.itKw*s.pue).toLocaleString()} unit="kW" sub="derived from PUE"/><Metric label="Peak inlet" value={s.peakC.toFixed(1)} unit="°C" sub="limit 32.0°C" warn/><Metric label="PUE" value={s.pue.toFixed(3)} sub="simulated"/><Metric label="Headroom" value={Math.max(0, Math.round(2160-s.itKw)).toString()} unit="kW" sub="rated capacity"/></div><div className="mt-4 grid gap-4 xl:grid-cols-[1.5fr_.8fr]"><section className="panel min-h-[430px] overflow-hidden"><div className="border-b border-slate-800 p-5"><h2 className="font-semibold">Synchronized facility twin</h2><p className="text-xs text-slate-500">Workload → power → heat → CDU-03 response</p></div><div className="twin-canvas"><div className="twin-grid"/><div className="twin-core"><Cpu size={28}/><b>GPU HALL</b><small>CLUSTER B · {s.itKw} kW</small></div>{["A01","A02","B01","B02","C01","C02"].map((r,i)=><button key={r} onClick={() => navigate(`/facilities/${facility.id}/incidents/inc-204`)} className={`rack rack-${i} ${i>2?"hot":""}`}><span>{r}</span><i style={{height:`${35+i*8}%`}}/></button>)}</div></section><aside className="space-y-4"><section className="panel p-5"><div className="eyebrow">FORECAST RISK</div><h2 className="mt-2 text-xl font-semibold text-amber-300">Watch condition</h2><p className="mt-4 text-sm leading-6 text-slate-400">B02 is forecast to cross its inlet limit if CDU-03 remains at current flow.</p><button onClick={() => navigate(`/facilities/${facility.id}/recommendations/rec-17`)} className="button primary mt-4 w-full justify-center">Review advisory <ArrowRight size={15}/></button></section><section className="panel p-5"><div className="eyebrow">OPERATING MODE</div><select value={s.mode} onChange={e=>s.setMode(e.target.value as typeof s.mode)} className="select mt-4 w-full"><option>Observe</option><option>Shadow</option><option>Advisory</option></select><p className="mt-3 text-xs leading-5 text-slate-500">Human approval is always required. No OT commands are issued.</p></section></aside></div></Shell>;
}
...
function Recommendation({ data, facility }: { data: SessionData; facility: Facility }) {
  ...
  const approve=async()=>{setError("");try{await api(`/api/facilities/${facility.id}/audit`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({action:"APPROVE_ADVISORY",simulatedAt:s.simulatedAt,safetyEvaluationId:evaluationId,payload:{recommendationId:"rec-17",outcome:"ALLOWED_AS_ADVISORY",command:{assetId:"cdu-03",flowPercent:78,durationMinutes:20},provenance:"SIMULATED"}})});setRecorded(true);}catch(e){setError(e instanceof Error?e.message:"Decision failed");}};
  return <Shell data={data} facility={facility}><PageHead eyebrow="TRUST COCKPIT / REC-17" title="Pre-emptive flow adjustment" detail="Same-input synthetic counterfactual; advisory only." action={<Status tone={recorded?"good":"warn"}>{recorded?"RECORDED":"AWAITING REVIEW"}</Status>}/><div className="grid gap-4 xl:grid-cols-[1.1fr_.9fr]"><section className="panel p-6"><div className="eyebrow">WHAT / WHY / WHERE / EFFECT</div><h2 className="mt-3 text-xl font-semibold">Increase CDU-03 flow to 78% for 20 minutes</h2><p className="copy">Reduce predicted B02 peak inlet by 1.6°C before the GPU Training Ramp reaches steady state.</p><div className="mt-6 grid gap-3 sm:grid-cols-2"><Metric label="Baseline peak" value="33.4" unit="°C" sub="19 constraint minutes" warn/><Metric label="Advisory peak" value="31.8" unit="°C" sub="0 constraint minutes"/></div></section><aside className="panel p-6"><div className="flex items-center gap-3"><ShieldCheck className="text-teal-300"/><h2 className="font-semibold">Safety Shield</h2></div>{["Command envelope","Cooling headroom","Maintenance state","Model confidence"].map(x=><div className="check-row" key={x}><span className={`check ${checked?"pass":""}`}>{checked?<Check size={12}/>:<Clock3 size={12}/>}</span>{x}</div>)}<button onClick={evaluate} className="button secondary mt-5 w-full justify-center">{checked?"Server evaluation passed":"Run safety evaluation"}</button>{facility.can_operate?<button disabled={!checked||recorded} onClick={approve} className="button primary mt-2 w-full justify-center">{recorded?"Decision recorded":"Approve advisory"}</button>:<p className="mt-4 rounded border border-amber-800/40 bg-amber-950/20 p-3 text-xs text-amber-200">Read-only role: operator permission is required to record a decision.</p>}{error&&<p role="alert" className="mt-3 text-xs text-red-300">{error}</p>}<button onClick={()=>navigate(`/facilities/${facility.id}/audit`)} className="mt-4 w-full text-xs text-cyan-300">View audit history</button></aside></div></Shell>;
}
...
function AuditPage({ data, facility }: { data: SessionData; facility: Facility }) {
  ...
  return <Shell data={data} facility={facility}><PageHead eyebrow="AUDIT HISTORY" title="Reconstructable decisions" detail="Immutable records filtered by your facility permission."/><section className="panel overflow-hidden">{records.map(r=><article key={r.id} className="facility-row"><div><Status>{r.action.replace(/_/g," ")}</Status><h2 className="mt-2 font-semibold">Recommendation {r.payload.recommendationId}</h2><p className="mt-1 text-xs text-slate-500">{new Date(r.created_at).toLocaleString()} · {r.model_version}</p></div><div className="text-right"><b>{r.payload.outcome}</b><p className={`${mono} mt-1 text-[10px] text-slate-500`}>{formatSimulatedAt(r.simulated_at)}</p></div></article>)}{records.length===0&&!error&&<p className="p-6 text-sm text-slate-500">No operator decisions have been recorded yet.</p>}{error&&<p role="alert" className="p-6 text-red-300">{error}</p>}</section></Shell>;
}
...
function Workspace({ data, facility, kind }: { data: SessionData; facility: Facility; kind: string }) {
  ...
  return <Shell data={data} facility={facility}><PageHead eyebrow={kind.toUpperCase()} title={names[kind]??"Operator guide"} detail="Synchronized synthetic context; no closed-loop control."/><div className="grid gap-4 lg:grid-cols-2"><section className="panel p-6"><Thermometer className="text-amber-300"/><h2 className="mt-4 text-xl font-semibold">{kind==="incidents"?"CDU-03 response lag during GPU Training Ramp":"SFO-01 operating representation"}</h2><p className="copy">Trace workload, thermal propagation, cooling response, model provenance, and safety constraints from one versioned scenario.</p></section><section className="panel p-6"><GitBranch className="text-cyan-300"/><h2 className="mt-4 font-semibold">Operating thread</h2><button onClick={()=>navigate(`/facilities/${facility.id}/recommendations/rec-17`)} className="action-row">Open recommendation <ArrowRight size={14}/></button><button onClick={()=>navigate(`/facilities/${facility.id}/operations`)} className="action-row">Return to operations <ArrowRight size={14}/></button></section></div></Shell>;
}
