    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(0,1fr)_390px]">
      <section className="panel p-5">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2">
          {visibleAssets.map((asset) => <button type="button" key={asset.id} onClick={() => selectAsset(asset)} className={`subpanel text-left transition hover:border-cyan-400/50 ${asset.id === assetId ? "border-cyan-400/70" : ""}`}>
            <div className="flex items-start justify-between gap-3"><div><div className="eyebrow">{asset.kind.replace(/_/g, " ")}</div><h2 className="mt-1 font-semibold text-slate-100">{asset.name}</h2></div><Status tone={asset.location.status === "RESOLVED" ? "good" : "warn"}>{asset.location.status}</Status></div>
            <p className="mt-2 text-xs text-slate-400">{asset.id} · {asset.location.zoneName ?? "No mapped physical location"}</p>
            <p className="mt-2 text-[11px] text-slate-500">{asset.manufacturer || asset.model ? `${asset.manufacturer ?? "Unknown manufacturer"} ${asset.model ?? ""}` : "No manufacturer/model source supplied"}</p>
          </button>)}
        </div>
      ...
      <aside className="panel p-5">
        {!selectedAsset ? <><div className="eyebrow">CANONICAL RECORD</div><h2 className="mt-2 text-lg font-semibold">Select an asset</h2><p className="mt-2 text-sm leading-6 text-slate-400">Search or choose a physical, cooling, power, or telemetry-supported asset. Every result is resolved from the active published model, not an old seeded inventory row.</p></> : <>
          ...
          <h2 className="mt-2 text-xl font-semibold">{selectedAsset.name}</h2><p className="mt-1 font-mono text-[11px] text-cyan-300">{selectedAsset.id}</p>
