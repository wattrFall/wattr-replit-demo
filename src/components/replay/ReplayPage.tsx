...
function SpatialTwin({
  ...
  return <div className="subpanel overflow-x-auto">
    <div className="flex items-center justify-between gap-2"><b className="text-sm">{title}</b><span className="text-xs text-slate-500">overhead rack grid · simulated inlet °C</span></div>
    ...
    <p className="mt-2 text-xs text-slate-500">{candidate ? `Outlined rack ${relocatedRackId} is the proposed relocated asset; labels show candidate delta against the same timestamp baseline.` : "Rack IDs and locations come from the immutable baseline model snapshot."}</p>
  </div>;
}
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(330px,.75fr)_minmax(0,1.25fr)]">
      <section className="space-y-4">
        <section className="panel p-5">
          ...
          <p className="mt-3 text-xs leading-5 text-slate-500">Synthetic demo data is explicitly marked and stored separately. It is never represented as uploaded facility history.</p>
